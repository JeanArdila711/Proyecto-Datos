...
def main(cantidad):
    ...
    print(f"Personas más amigas: {', '.join([f'Persona {persona}' for persona in personas_mas_amigas])}")

    persona_con_mas_amigos = red_social.persona_con_mas_amigos()
    print(f"Persona con más amigos: Persona {persona_con_mas_amigos}")

    relacion_mas_fuerte = red_social.relacion_mas_fuerte()
    if relacion_mas_fuerte:
        print(f"Relación más fuerte: Persona {relacion_mas_fuerte[0]} y Persona {relacion_mas_fuerte[1]}")
    else:
